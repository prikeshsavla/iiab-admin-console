...
var getLangCodes = $.getJSON(consoleJsonDir + "lang_codes.json")
  .done(function (data) {
    langCodes = data;
    for (var lang in langCodes) langCodesIndex[langCodes[lang]["iso2"]] = lang;
...
function procPage() {
  ...
  if (!menuParams.allow_poweroff) {
    ...
    gEBI("poweroffLink").innerHTML =
      "<strong>" + menuParams.poweroff_prompt + "</strong>";
  ...
  } else $("#headerDesktop").css("display", "flex");
}

function updateServerTime() {
  if (
    isMobile ||
    navigator.userAgent.search("Win64") !== -1 ||
...
function createScaffold() {
  ...
  for (var i = 0; i < menuItems.length; i++) {
    ...
    html +=
      '<div id="' +
      menuItemDivId +
      '" class="flex-row content-item" dir="auto">&emsp;Attempting to load ' +
      menu_item_name +
      " </div>";
  }
  $("#" + menuDivId).html(html);
...
function drawMenu() {
  ...
  for (var i = 0; i < menuItems.length; i++) {
    ...
    if (menuDefs[menuItems[i]].hasOwnProperty("menu_item_name"))
      // already loaded
      procMenuItem(menuDefs[menuItems[i]]);
    else getMenuDef(menuItems[i]);
  }
}
...
function calcItemVerbosity() {
  ...
  if (toggleDisplay) showMobile = !showMobile;
  if (showMobile) {
    if (menuParams.mobile_incl_description) showDescription = true;
    if (menuParams.mobile_incl_extra_description) showExtraDescription = true;
    if (menuParams.mobile_incl_extra_html) showExtraHtml = true;
    if (menuParams.mobile_incl_footnote) showFootnote = true;
  } else {
    if (menuParams.desktop_incl_description) showDescription = true;
    if (menuParams.desktop_incl_extra_description) showExtraDescription = true;
    if (menuParams.desktop_incl_extra_html) showExtraHtml = true;
    if (menuParams.desktop_incl_footnote) showFootnote = true;
  }
}
    ...
    .done(function (data) {
      ...
      if (menuDefs[menuItem].hasOwnProperty("zim_name")) {
        ...
        for (var i = 0; i < zimSubstParams.length; i++) {
          ...
          if (
            zimVersions.hasOwnProperty(zimName) &&
...
function procMenuItem(module) {
  ...
  if (selectedLangs.length > 0 && selectedLangs.indexOf(module.lang) == -1) {
    // not a selected language
    $(menuItemDivId).hide();
    return;
  }
  $(menuItemDivId).show();
  ...
  else if (module["intended_use"] == "cups") menuHtml += calcCupsLink(module);
  ...
  else if (module["intended_use"] == "map") menuHtml += calcMapLink(module);
...
function calcZimLink(module) {
  ...
  var href = "";
  if (
    zimVersions.hasOwnProperty(module.zim_name) &&
    ...
    href =
      ...
      module.menu_item_name +
      "&zim_name=" +
      module.zim_name; //not defined in zimVersions
...
function calcKolibriLink(module) {
  ...
  if (menuConfig.hasOwnProperty("kolibriUrl"))
    ...
    else href = host + ":" + menuConfig.kolibriPort + "/" + href;
  }
...
function calcMapLink(module) {
  ...
  if (
    osmVersions.hasOwnProperty(module.map_name) &&
...
function calcDownloadLink(module) {
  ...
  if (module.hasOwnProperty("download_file"))
    // treat specific file as a start page for downloads
...
function calcItemHtml(href, module) {
  ...
  if (href != undefinedPageUrl) {
    if (module.hasOwnProperty("start_url") && module.start_url != "") {
      if (module.intended_use == "external")
        ...
        else startPage = startPage + module["start_url"];
      }
    }
  }
  ...
  if (showDescription) {
    ...
    if (module.hasOwnProperty("apk_file") && include_apk_links) {
      ...
      if (menuConfig["apkLinkPhrase"].hasOwnProperty(module.lang))
        ...
        html +=
          '<p>Click here to download <a href="' +
          apkBaseUrl +
          module.apk_file +
          '">' +
          module.apk_file +
  ...
  if (showExtraDescription) html += getTextField(module, "extra_description");
  consoleLog("href = " + href);
  html +=
    '<div id="' +
    module.menu_id +
    '-htmlf" class="content-extra toggleExtraHtml"></div>'; // scaffold for extra html
  if (showFootnote) {
    var footnote = getTextField(module, "footnote", false);
    if (footnote != "") html += "<p><small>" + footnote + "</small></p>";
  }
  html += "</div></div></div>";
...
function substitute(instr, module) {
  for (var i = 0; i < zimSubstParams.length; i++) {
    field = zimSubstParams[i];
    if (field === "article_count" || field === "media_count")
      //  && typeof module[field] === "string"
      // 9999999 -> 9,999,999 or 9.999.999 or 9 999 999 in footnotes, depending on locale
      instr = instr.replace(
        substRegEx[field],
        Number(module[field]).toLocaleString()
      );
    else instr = instr.replace(substRegEx[field], module[field]);
  }
  return instr;
}
...
function getExtraHtml(module) {
  if (
    showExtraHtml &&
    module.hasOwnProperty("extra_html") &&
    module["extra_html"] != ""
  ) {
...
function getKiwixSearch() {
  ...
  var resp = $.get("/kiwix/search", searchArg).always(function (responseText) {
...
function parseKiwixSearchResults(searchRes) {
  ...
  parsedResults["header"] = searchRes.substring(
    header + '<div class="header">'.length,
    searchRes.indexOf("</div>", header)
  );
  parsedResults["results"] = searchRes.substring(
    results + '<div class="results">'.length,
    searchRes.lastIndexOf("</div>", footer)
  );
  parsedResults["footer"] = searchRes.substring(
    footer + '<div class="footer">'.length,
    searchRes.indexOf("</div>", footer)
  );
  return parsedResults;
}

function getKiwixSearch2(searchStr) {
  // not used
  var resp = $.ajax({
    type: "GET",
    async: true,
...
function buildHeaders() {
  if (menuParams.show_headers === true) {
    ...
    $("#heading-navbar").addClass(
      "navbar navbar-expand-lg navbar-light bg-transparent justify-content-center"
    );
    ...
    for (var i = 0; i < menuItems.length; i++) {
      ...
      if (module.hasOwnProperty("header")) {
        ...
        if (header) {
          var navHtml = `<li class="nav-item "> <a class="nav-link text-dark font-weight-bold" href="#${module["menu_id"]}">${module["title"]}</a></li>`;
...
function setLocalStore() {
  ...
  else localStorage.setItem("selected_langs", "");
...
function genLangSelector() {
  ...
  for (var i in menuItems) {
    ...
    consoleLog(lang);
    if (dupsCtrl.indexOf(lang) != -1) continue;
    else {
      ...
      if (isMobile) html += largeCheckStyle;
      html += 'name="' + lang + '"';
      if (selectedLangs.indexOf(lang) != -1) html += " checked";
      html +=
        ">&nbsp;&nbsp;<span>" +
        langName +
        "</span><span> (" +
